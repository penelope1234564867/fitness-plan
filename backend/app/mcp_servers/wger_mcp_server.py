...
class ListMusclesInput(BaseModel):
    """列出所有肌群。"""
    model_config = ConfigDict(str_strip_whitespace=True)
...
class SearchExercisesInput(BaseModel):
    ...
    query: Optional[str] = Field(
        default=None,
        description="关键词搜索（匹配动作名称和描述），例如 'bench press'、'crunch'、'squat'",
        max_length=200,
    )
    muscle: Optional[int] = Field(
        default=None,
        description="目标肌群 ID（从 list_muscles 获取），例如 4=Chest、1=Biceps",
        ge=1,
    )
    equipment: Optional[int] = Field(
        default=None,
        description="器材 ID（从 list_equipment 获取），例如 3=Dumbbell、1=Barbell",
        ge=1,
    )
    category: Optional[int] = Field(
        default=None,
        description="分类 ID（从 list_categories 获取），例如 11=Chest、10=Abs",
        ge=1,
    )
    limit: Optional[int] = Field(
        default=DEFAULT_LIMIT,
        description=f"返回结果数量上限（1-{MAX_LIMIT}）",
        ge=1,
        le=MAX_LIMIT,
    )
...
async def wger_search_exercises(params: SearchExercisesInput) -> str:
    ...
    try:
        ...
        if params.muscle:
            api_params["muscles"] = params.muscle  # 注意是复数！
        if params.equipment:
            api_params["equipment"] = params.equipment
        if params.category:
            api_params["category"] = params.category
        api_params["limit"] = min(params.limit or DEFAULT_LIMIT, MAX_LIMIT)
        api_params["offset"] = params.offset or 0
...
class GetExerciseDetailsInput(BaseModel):
    ...
    exercise_id: int = Field(
        ...,
        description="动作 ID（从 search_exercises 返回的 id 字段获取）",
        ge=1,
    )
...
async def wger_get_exercise_details(params: GetExerciseDetailsInput) -> str:
    """获取单个训练动作的完整详细信息。
    ...
    """
    try:
        data = await _get(f"exerciseinfo/{params.exercise_id}")
        import json
        return json.dumps(data, ensure_ascii=False, default=str)
    except httpx.HTTPStatusError as e:
        if e.response.status_code == 404:
            return f"Error: 动作 ID {params.exercise_id} 不存在"
...
class GetExerciseImagesInput(BaseModel):
    """获取动作的教学图片。"""
    model_config = ConfigDict(str_strip_whitespace=True)
...
async def wger_get_exercise_images(params: GetExerciseImagesInput) -> str:
    """获取指定训练动作的教学图片列表。
    ...
    """
    try:
        data = await _get("exerciseimage", {"exercise": params.exercise_id})
        import json
        ...
        if e.response.status_code == 404:
            return f"Error: 动作 ID {params.exercise_id} 不存在或没有图片"
        return f"Error: API 请求失败 (HTTP {e.response.status_code})"
    except Exception as e:
...
class GetExerciseVideosInput(BaseModel):
    """获取动作的演示视频。"""
    model_config = ConfigDict(str_strip_whitespace=True)
...
async def wger_get_exercise_videos(params: GetExerciseVideosInput) -> str:
    """获取指定训练动作的演示视频列表。
    ...
    """
    try:
        data = await _get("video", {"exercise": params.exercise_id})
        import json
        ...
        if e.response.status_code == 404:
            return f"Error: 动作 ID {params.exercise_id} 不存在或没有视频"
        return f"Error: API 请求失败 (HTTP {e.response.status_code})"
    except Exception as e:
